...
function rcp_mailchimp_settings_menu() {
	// add settings page
	add_submenu_page( 'rcp-members', __( 'Restrict Content Pro Mail Chimp Settings', 'restrict-content-pro-mailchimp' ), __( 'Mail Chimp', 'restrict-content-pro-mailchimp' ), 'manage_options', 'rcp-mailchimp', 'rcp_mailchimp_settings_page' );
}
	...
	<div class="wrap">
		...
		<form method="post" action="options.php" class="rcp_options_form">
			...
			<table class="form-table">
				...
				<tr>
					<th>
						<label for="rcp_mailchimp_settings[mailchimp_api]"><?php _e( 'Mail Chimp API Key', 'restrict-content-pro-mailchimp' ); ?></label>
					</th>
					<td>
						...
						<div class="description"><?php _e( 'Enter your Mail Chimp API key to enable a newsletter signup option with the registration form.', 'restrict-content-pro-mailchimp' ); ?></div>
					</td>
				</tr>
				<tr>
					<th>
...
function rcp_add_mc_table_column_header_and_footer() {
	echo '<th style="width: 140px;">' . __ ( 'Mail Chimp Signup', 'restrict-content-pro-mailchimp' ) . '</th>';
}
...
function rcp_add_mc_table_column_content( $user_id ) {
	...
	if ( $signed_up ) {
		$signed_up = __( 'Yes', 'restrict-content-pro-mailchimp' );
	} else {
		$signed_up = __( 'No', 'restrict-content-pro-mailchimp' );
	}
